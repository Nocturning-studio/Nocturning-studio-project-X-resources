uniform sampler2D s_smap_depth;
...
float PCF_Filter (float4 uv, float zReceiver, float filterRadiusUV)
{
	float sum = 0.0f;
...
float PCSS (sampler2D shadowMapTex, float4 coords)
{
	...
	float numBlockers = 0;
	FindBlocker (avgBlockerDepth, numBlockers, coords, zReceiver);
	if (numBlockers < 1)
	{
	...
	else
	{
		...
		// STEP 3: filtering
		return PCF_Filter(coords, zReceiver, filterRadiusUV);
	}
}