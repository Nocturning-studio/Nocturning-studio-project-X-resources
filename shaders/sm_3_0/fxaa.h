...
#elif (FXAA_BRIGHTNESS_AS_LUMA == 1)
FxaaFloat FxaaLuma(FxaaFloat4 rgba)
{
    return max(rgba.x, max(rgba.y, rgba.z)); //(rgba.x + rgba.y + rgba.z) / 3;
}
#else
FxaaFloat FxaaLuma(FxaaFloat4 rgba)
{
...
#define lumaM rgbyM.y
    ...
    // STALKER compat hacks
    FxaaFloat4 output;
    if (earlyExit)
        output = rgbyM;
    else
        output = FxaaFloat4(FxaaTexTop(tex, posM).xyz, lumaM);
